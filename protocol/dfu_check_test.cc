...
constexpr char kTestData[] = "protocol/test/test.fwupdate";
...
TEST_F(LibHothTest, dfu_check_test) {
  ...
  libhoth_extract_ot_bundle(image, &rom_ext, &app);

  struct opentitan_get_version_resp mock_response = {};

  mock_response.rom_ext.slots[0].major = 0;
  mock_response.rom_ext.slots[0].minor = 116;
  mock_response.rom_ext.slots[1].major = 0;
  mock_response.rom_ext.slots[1].minor = 116;

  mock_response.app.slots[0].major = 0;
  mock_response.app.slots[0].minor = 2025102930;
  mock_response.app.slots[1].major = 0;
  mock_response.app.slots[1].minor = 2025102930;

  EXPECT_EQ(libhoth_dfu_check(&hoth_dev_, image, statbuf.st_size, &mock_response), LIBHOTH_OK);
...
TEST_F(LibHothTest, dfu_check_fail) {
  ...
  mock_response.rom_ext.slots[0].major = 0;
  mock_response.rom_ext.slots[0].minor = 115;
  mock_response.rom_ext.slots[1].major = 0;
  mock_response.rom_ext.slots[1].minor = 116;
  ...
  EXPECT_EQ(libhoth_dfu_check(&hoth_dev_, image, statbuf.st_size, &mock_response), -1);
